...
public class CarDriverServiceImpl implements CarDriverService{
	...
	@Override
	public List<CarDriver> find1(String id) throws Exception {
		CarDriverExample example = new CarDriverExample();
		return driverMapper.selectByExample(example);
	}
	@Override
	public List<CarDriver> gets() throws Exception {
		return driverMapper.find();
	}


	@Override
	public List<CarDriver> gets1(String id) throws Exception {
		return driverMapper.find1(id);
	}

	@Override
	public EUDataGridResult getList1(int page, int rows, CarDriver carDriver,String id) throws Exception {
		//分页处理
				PageHelper.startPage(page, rows);
				List<CarDriver> list = driverMapper.find1(id);
				//创建一个返回值对象
				EUDataGridResult result = new EUDataGridResult();
				result.setRows(list);
				//取记录总条数
	...
	public EUDataGridResult searchCarDriverByCarDrivernum(Integer page, Integer rows, String drivenum) throws Exception {
		//分页处理
		PageHelper.startPage(page, rows);
		List<CarDriver> list = driverMapper.searchCarDriverByCarDriverAA(drivenum);
		//创建一个返回值对象
		EUDataGridResult result = new EUDataGridResult();
		result.setRows(list);
		//取记录总条数
	...
	@Override
	public EUDataGridResult searchCarDriverByCarDriverName1(Integer page, Integer rows, String id,String name) throws Exception {
			//分页处理
			PageHelper.startPage(page, rows);
			List<CarDriver> list = driverMapper.searchCarDriverByCarDriverDriverfrom1(id,name);
			//创建一个返回值对象
			EUDataGridResult result = new EUDataGridResult();
			result.setRows(list);
			//取记录总条数
	...
	@Override
	public EUDataGridResult searchCarDriverByCarDrivernum1(Integer page, Integer rows,String id, String drivenum) throws Exception {
		//分页处理
		PageHelper.startPage(page, rows);
		List<CarDriver> list = driverMapper.searchCarDriverByCarDriverAA1(id,drivenum);
		//创建一个返回值对象
		EUDataGridResult result = new EUDataGridResult();
		result.setRows(list);
		//取记录总条数
	...
	@Override
	public CarDriver getDep(String string) throws Exception {
		return driverMapper.searchCarDriverByCarDepName(string);
	}
