...
<table class="easyui-datagrid" id="roleList" title="角色列表" 
    ...
    <thead>
        <tr>
            ...
            <th data-options="field:'roleEdit',align:'center',width:50,formatter:formatRoleEdit">编辑</th>
        </tr>
    </thead>
</table>
	...
	if(value == null || value == ''){
		...
		$("#roleList").datagrid({
	        ...
	        columns : [ [ 
	             	...
	             	{field : 'roleEdit', width : 50, align:'center', title : '编辑',formatter:formatRoleEdit}
	        ] ],  
	    });
	}else{
		$("#roleList").datagrid({  
...
function  formatRoleEdit(value, row, index){ 
	return "<a herf='#' style='cursor:pointer' icon='icon-edit' onclick='role_edit()'><span style='color:blue'>编辑</span></a>";
};
    ...
    function role_edit(){
    	$.get("role/edit_judge",'',function(data){
       		if(data.msg != null){
            	...
            	if(ids == '001' ||ids.substr(0, 3)=="001"){
            		$.messager.alert('提示','ID为001的用户角色不能被编辑!');
            		return ;
            	}
    ...
    function role_delete(){
    	$.get("role/delete_judge",'',function(data){
      		if(data.msg != null){
            	...
            	if(ids == '001' ||ids.substr(0, 3)=="001"){
            		$.messager.alert('提示','ID为001的用户角色不能被删除!');
            		return ;
            	}
