...
<table class="easyui-datagrid" id="taskList" title="运行状况列表" 
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'taskId',align:'center',width:60">时间</th>
            <th data-options="field:'workId',align:'center',width:70,formatter:formatTaskWork">路码表月终读数</th>
            <th data-options="field:'manufactureSn',align:'center',width:50,formatter:formatTaskManufacture">月公里</th>
            <th data-options="field:'taskQuantity',align:'center',width:50">趟数/天数</th>
            <th data-options="field:'workingHours',width:50,align:'center'">燃油/润滑油</th>
            <th data-options="field:'taskId1',align:'center',width:50">百公里耗油</th>
            <th data-options="field:'workId1',align:'center',width:100,formatter:formatTaskWork">路/桥/停车费</th>
            <th data-options="field:'manufactureSn1',align:'center',width:100,formatter:formatTaskManufacture">备注</th>
        </tr>
    </thead>
</table>
...
<div  id="toolbar_task" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_task" style="float: right;">
        ...
        <div id="menu_task" style="width:120px"> 
			<div data-options="name:'taskId'"></div> 
			<div data-options="name:'taskWorkId'"></div>
			<div data-options="name:'taskManufactureSn'"></div> 
		</div>     
    </div>  

</div>  
<div id="taskEditWindow" class="easyui-window" title="编辑状况记录" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'task/edit'" style="width:50%;height:55%;padding:10px;">
</div>
<div id="taskAddWindow" class="easyui-window" title="添加状况记录" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'task/add'" style="width:50%;height:55%;padding:10px;">
</div>
...
function doSearch_task(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		...
		$("#taskList").datagrid({
	        title:'运行状况列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get', nowrap:true,  
	        toolbar:"toolbar_task", url:'task/list', method:'get', loadMsg:'数据加载中......', fitColumns:true,//允许表格自动缩放,以适应父容器  
	        columns : [ [ 
	             	{field : 'ck', checkbox:true }, 
	             	{field : 'taskId', width : 60, title : '时间', align:'center'},
	             	{field : 'workId', width : 80, align : 'center', title : '路码表月终读数', formatter:formatTaskWork},
	             	{field : 'manufactureSn', width : 50, align : 'center', title : '月公里', formatter:formatTaskManufacture}, 
	             	{field : 'workingHours', width : 50, title : '趟数/天数', align:'center'}, 
	             	{field : 'unitPrice', width : 50, title : '燃油/润滑油', align:'center'}, 
	             	{field : 'taskId1', width : 50, title : '百公里耗油', align:'center'},
	             	{field : 'workId1', width : 100, align : 'center', title : '路/桥/停车费'},
	             	{field : 'manufactureSn1', width : 100, align : 'center', title : '备注'}, 
	        ] ],  
	    });
	}else{
		$("#taskList").datagrid({  
