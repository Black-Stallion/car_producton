...
<table class="easyui-datagrid" id="carDeployList" title="变动记录信息" 
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'deployEdit',align:'center',width:50,formatter:formatDeployEdit">编辑</th>
        </tr>
    </thead>
</table> 
	...
	if(value == null || value == ''){
		$("#carDeployList").datagrid({
	        ...
	        columns : [ [
	            	...
	            	{field : 'deployEdit', width : 50, align:'center', title : '编辑',formatter:formatDeployEdit}
	       ]  ],  
	    });
	}else{
		$("#carDeployList").datagrid({
...
function  formatDeployEdit(value, row, index){ 
	return "<a herf='#' style='cursor:pointer' icon='icon-edit' onclick='carDeploy_edit()'><span style='color:blue'>编辑</span></a>";
};
//格式化违规记录记录信息
