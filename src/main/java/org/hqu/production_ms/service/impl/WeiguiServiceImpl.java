...
public class WeiguiServiceImpl implements WeiguiService {
	...
	@Override
	public List<Weigui> find1(String id) throws Exception {
		WeiguiExample example = new WeiguiExample();
		return weiguiMapper.selectByExample(example);
	}

	@Override
	public EUDataGridResult getList1(int page, int rows, Weigui weigui,String id) throws Exception {
		//分页处理
				PageHelper.startPage(page, rows);
				List<Weigui> list = weiguiMapper.find1(id);
				//创建一个返回值对象
				EUDataGridResult result = new EUDataGridResult();
				result.setRows(list);
				//取记录总条数
	...
	@Override
	public EUDataGridResult searchWeiguiBycarnum(Integer page, Integer rows, int carnum) throws Exception {
		//分页处理
		PageHelper.startPage(page, rows);
