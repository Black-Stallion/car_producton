...
<div style="padding: 10px 10px 10px 10px">
	<form id="carNowSendAddForm" class="carNowSendForm" method="post">
		<center>
			<table class="border-table" cellpadding="5" align="center">
				...
				<tr>
					...
					<td><input class="easyui-combobox" id="departmenCombox1726" name="departmentName" data-options="required:true" panelHeight="200" size="15px" /> 
						<input id="departmentFrist731" type="hidden" /> 
						<input class="easyui-combobox" id="xxdepartment1726" panelHeight="200"  size="15px" /> 
						<input id="departmentId1726" type="hidden" />
						<input name="departmentId" id="departmentId2726" type="hidden" />
						<!-- 
						<input class="easyui-combobox" id="departmenCombox1726" 
						data-options="required:true" panelHeight="auto" size="7px" /> <input
						id="departmentFrist731" type="hidden" /> <input
						class="easyui-combobox" id="xxdepartment1726" panelHeight="200" name="departmentName"
						size="12px" /> <input id="departmentId1726"
						type="hidden" />
						<input
						class="easyui-combobox" id="xxdepartment2726" panelHeight="auto"
						size="10px" /> <input name="departmentId" id="departmentId2726"
						type="hidden" />
						 -->
						</td>
					...
					<td><input class="easyui-combobox"
						id="xjzcarCombox3726" data-options="required:true" name="usenature"
						panelHeight="auto" size="9px" /> <input name="usenature"
						id="usenature3726" type="hidden" /> <input class="easyui-combobox" name="carnumber"
						id="xjzcarNumberCombox3726" data-options="required:true"
						panelHeight="100" size="10px" /> <input name="carId" id="carId3726"
						type="hidden" /></td>
				</tr>
				<tr>
	...
	<div style="padding: 5px 0 0 700px">
		<a href="javascript:void(0)" class="easyui-linkbutton"
			onclick="submitCarSendAddForm()">提交</a> <a href="javascript:void(0)"
			class="easyui-linkbutton" onclick="clearCarSendADDForm()">重置</a>
	</div>
</div>
<script type="text/javascript">
	...
	$(function() {
		...
		orderAddEditor3 = KindEditor.create("#carNowSendAddForm [name=road]",{
			resizeType : 1,
	...
	function submitCarSendAddForm() {
		//有效性验证
		if (!$('#carNowSendAddForm').form('validate')) {
		...
		//ajax的post方式提交表单
		//$("#carNowSendAddForm").serialize()将表单序列号为key-value形式的字符串
		$.post("carSend/insertnow", $("#carNowSendAddForm").serialize(), function(
				data) {
			if (data.status == 200) {
				$.messager.alert('提示', '新增派车单成功'+data.msg);
				clearCarSendADDForm();
				$("#carNowSendAddWindow").window('close');
	...
	$('#xjzcarCombox3726').combobox({
		...
		onSelect : function(data) {
			$('#usenature3726').val(data.usenature);
			var fatherdep = $('#usenature3726').val();

			$('#xjzcarNumberCombox3726').combobox({
				...
				onSelect : function(data1) {
					$('#carId3726').val(data1.carId);
				}

			}).combobox('clear');
		}
	});
	$('#departmenCombox1726').combobox({
	...
	//	panelHeight : 'auto',
		onSelect : function(data) {
			$('#departmentFrist731').val(data.departmentId);
			$('#departmentId1726').val(data.departmentId);
			$('#departmentId2726').val(data.departmentId);
			var fatherdep = $('#departmentFrist731').val();
			$('#xxdepartment1726').combobox({
				url : "department/get_data_second/" + fatherdep,
				valueField : 'departmentId',
				textField : 'departmentName',
				editable : false,
				onSelect : function(data1) {
					$('#departmentId1726').val(data1.departmentId);
					$('#departmentId2726').val(data1.departmentId);
				//	var fatherdep = $('#departmentId1726').val();
				//	$('#xxdepartment2726').combobox({
				//		url : "department/get_data_third/" + fatherdep,
				//		valueField : 'departmentId',
				//		textField : 'departmentName',
				//		editable : false,
				//		onSelect : function(data2) {
				//			$('#departmentId2726').val(data2.departmentId);
				//		}
				//	}).combobox('clear');
				}
