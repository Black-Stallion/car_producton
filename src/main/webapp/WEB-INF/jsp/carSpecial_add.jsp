...
<div style="padding:10px 10px 10px 10px">
	<form id="carSpecialAddForm" class="carSpecialForm" method="post">
	 <center>
	    <table  class="border-table" cellpadding="5" algin="center">
	        ...
	        <tr>
	            ...
	            <td>
	            	<input class="easyui-combobox" id="departmenCombox151" data-options="required:true" panelHeight="150" />
	            	<input name="departmentFrist" id="departmentFrist" type="hidden"/>
    				<input name="departmentId" id="departmentId97" type="hidden"/>
    				<!-- 
    				<input class="easyui-combobox" id="departmenCombox151" data-options="required:true" panelHeight="auto" size="8px" />
	            	<input name="departmentFrist" id="departmentFrist" type="hidden"/>
    				<input class="easyui-combobox" id="department151"  panelHeight="200" size="11px"/>  
    				<input name="departmentId" id="departmentId97" type="hidden"/>
    				 -->
	            </td>
		      </tr>
	        ...
	        <tr >
	            ...
	            <td colspan="3" >
	            	 <a href="javascript:void(0)" class="easyui-linkbutton specialPicFileUpload">上传图片</a>
	                 <input type="hidden" id="specialimage" name="image"/>
	            </td>
	        </tr>
	    </table>
	     </center>
	    <input type="hidden" name="carSpecialParams"/>
	</form>
	<div style="padding:5px 0 0 650px">
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitCarSpecialAddForm()">提交</a>
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="clearCarSpecialADDForm()">重置</a>
	</div>
</div>
...
$(function(){
	...
	//初始化类目选择和图片上传器
	TAOTAO.initSpecialPicFileUpload({fun:function(node){
		//根据产品的分类id取产品 的规格模板，生成规格信息。第四天内容。
		TAOTAO.changeItemParam(node, "carSpecialAddForm");
	}});
});
	
	//提交表单
	function submitCarSpecialAddForm(){
		//有效性验证
		if(!$('#carSpecialAddForm').form('validate')){
			$.messager.alert('提示','表单还未填写完成!');
			return ;
		}
		$.post("carPublic/check",$("#carSpecialAddForm").serialize(),function(data){
       		if(data.msg != null){
       			$.messager.alert('提示', data.msg);
       		}else{
		$.post("carSpecial/insert",$("#carSpecialAddForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示','新增车辆成功!');
				$("input[name='image']").val(null);
				clearCarSpecialADDForm();
				$("#carSpecialAddWindow").window('close');
	...
	$('#departmenCombox151').combobox({  
      ...
      //  panelHeight:'auto',  
        onSelect : function(data){  
            ...
            $('#departmentId97').val(data.departmentId);  
      //      var fatherdep= $('#departmentFrist').val();
     //       $('#department151').combobox({  
      //          url:"department/get_data_second/"+fatherdep,  
      //          editable:false,  
      //          valueField:'departmentId',  
      //          textField:'departmentName' ,
      //          onSelect : function(data1){  
      //            $('#departmentId97').val(data1.departmentId);  
      //          }
      //      }).combobox('clear');  
        }  
    });  
	
	function clearCarSpecialADDForm(){
		$('#carSpecialAddForm').form('reset');
	}
