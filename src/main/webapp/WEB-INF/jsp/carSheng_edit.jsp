...
<div style="padding:10px 10px 10px 10px">
	<form id="carShengEditForm" class="carShengForm" method="post">
	    ...
	    <table  class="border-table" cellpadding="5" algin="center">
	        ...
	        <tr>
	            ...
	            <td>
	            	<input class="easyui-combobox" id="departmenComboxzx"  name="departmentName" panelHeight="200" />
	            	<input name="departmentFrist" id="departmentFrist" type="hidden"/>
    				<input name="departmentId" id="departmentId12" type="hidden"/>
    				<!-- 
    				<input class="easyui-combobox" id="departmenComboxzx"  value="${fathername }"panelHeight="auto" size="8px" />
	            	<input name="departmentFrist" id="departmentFrist" type="hidden"/>
    				<input class="easyui-combobox" id="departmentzx" name="departmentName" panelHeight="200" size="11px"/>  
    				<input name="departmentId" id="departmentId12" type="hidden"/>
    				 -->
	            </td>
		      </tr>
	...
	<div style="padding:5px 0 0 650px">
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitCarShengEditForm()">提交</a>
	</div>
</div>
<script type="text/javascript">
	
	function submitCarShengEditForm(){
		if(!$('#carShengEditForm').form('validate')){
			$.messager.alert('提示','表单还未填写完成!');
			return ;
		}
		
		$.post("carSheng/update_all",$("#carShengEditForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示','修改车辆信息成功!','info',function(){
					$("input[name='image']").val(null);
					$("#carShengEditWindow").window('close');
	    ...
	    //    panelHeight:'auto',  
	        onSelect : function(data){  
	            ...
	            $('#departmentId12').val(data.departmentId);  
	    //        var fatherdep= $('#departmentFrist').val();
	    ///        $('#departmentzx').combobox({  
	    //            url:"department/get_data_second/"+fatherdep,  
	    //            editable:false,  
	    //            valueField:'departmentId',  
	    //            textField:'departmentName' ,
	    //            onSelect : function(data1){  
	    //              $('#departmentId12').val(data1.departmentId);  
	   //             }
	    //        }).combobox('clear');  
	        }  
	    });  
