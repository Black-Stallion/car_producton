...
public class DriverRecordServiceImpl implements DriverRecordService {
	...
	@Override
	public List<DriverRecord> find1(String id) throws Exception {
		DriverRecordExample example = new DriverRecordExample();
		return driverRecordMapper.selectByExample(example);
	}

	@Override
	public EUDataGridResult getList1(int page, int rows, DriverRecord driverRecord,String id) throws Exception {
		//分页处理
				PageHelper.startPage(page, rows);
				List<DriverRecord> list = driverRecordMapper.find1(id);
				//创建一个返回值对象
				EUDataGridResult result = new EUDataGridResult();
				result.setRows(list);
				//取记录总条数
