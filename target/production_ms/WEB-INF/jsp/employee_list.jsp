...
<table class="easyui-datagrid" id="employeeList" title="司机列表" 
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'empId',align:'center',width:100">司机编号</th>
            <th data-options="field:'empName',align:'center',width:100">司机姓名</th>
            <th data-options="field:'sex',align:'center',width:100,formatter:TAOTAO.formatSex">性别</th>
            <th data-options="field:'department',align:'center',width:100,formatter:formatDepartment">所属车间</th>
            <th data-options="field:'idCode',align:'center',width:150">准驾车型</th>
            <th data-options="field:'education',align:'center',width:100">驾驶证号</th>
            <th data-options="field:'degree',align:'center',width:100">初次领证</th>
            <th data-options="field:'major',align:'center',width:100">发证机关</th>
            <th data-options="field:'educationForm',align:'center',width:100">文化程度</th>
            <th data-options="field:'birthday',width:130,align:'center',formatter:TAOTAO.formatDate">出生日期</th>
            <th data-options="field:'joinDate',width:130,align:'center',formatter:TAOTAO.formatDate">任职时间</th>
            <th data-options="field:'status',width:100,align:'center'">司机状态</th>
        </tr>
    </thead>
</table>
...
<div  id="toolbar_employee" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_employee" style="float: right;">
        ...
        <div id="menu_employee" style="width:120px"> 
			<div data-options="name:'employeeId'">司机编号</div> 
			<div data-options="name:'employeeName'">司机名称</div>
			<div data-options="name:'departmentName'">车间名称</div> 
		</div>     
    </div>  

</div>  

<div id="employeeEditWindow" class="easyui-window" title="编辑司机" data-options="modal:true,
	closed:true,resizable:true,iconCls:'icon-save',href:'employee/edit'" style="width:40%;height:70%;padding:10px;">
</div>
<div id="employeeAddWindow" class="easyui-window" title="添加司机" data-options="modal:true,
	closed:true,resizable:true,iconCls:'icon-save',href:'employee/add'" style="width:40%;height:70%;padding:10px;">
</div>

<div id="empDepartmentInfo" class="easyui-dialog" title="车间信息" data-options="modal:true,
...
function doSearch_employee(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		...
		$("#employeeList").datagrid({
	        title:'司机列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get', nowrap:true,  
	        toolbar:"toolbar_employee", url:'employee/list', method:'get', loadMsg:'数据加载中......', fitColumns:true,//允许表格自动缩放,以适应父容器  
	        columns : [ [ 
	             	{field : 'ck', checkbox:true }, 
	             	{field : 'empId', width : 100, align:'center', title : '司机编号'},
	             	{field : 'empName', width : 100, align : 'center', title : '司机姓名'},
	             	{field : 'sex', width : 100, align : 'center', title : '性别', formatter:TAOTAO.formatSex}, 
	             	{field : 'department', width : 100, title : '所属车间', align:'center',formatter:formatDepartment}, 
	            	...
	            	{field : 'education', width : 100, title : '学历', align:'center'}, 
	             	...
	             	{field : 'status',  width : 100, title : '司机状态', align:'center'}
	        ] ],  
	    });
	}else{
		$("#employeeList").datagrid({  
		...
		$.get("department/edit_judge",'',function(data){
    		if(data.msg != null){
    			...
    			$.post("department/update_all",$("#empDepartmentEditForm").serialize(), function(data){
    				if(data.status == 200){
    					$.messager.alert('提示','修改车间成功!','info',function(){
    ...
    function employee_edit(){
    	$.get("employee/edit_judge",'',function(data){
       		if(data.msg != null){
            	...
            	if(ids.length == 0){
            		$.messager.alert('提示','必须选择一个司机才能编辑!');
            		return ;
            	}
            	if(ids.indexOf(',') > 0){
            		$.messager.alert('提示','只能选择一个司机!');
            		return ;
            	}
    ...
    function employee_delete(){
    	$.get("employee/delete_judge",'',function(data){
      		if(data.msg != null){
      			...
      			var ids = getEmployeeSelectionsIds();
            	if(ids.length == 0){
            		$.messager.alert('提示','未选中司机!');
            		return ;
            	}
            	$.messager.confirm('确认','确定删除ID为 '+ids+' 的司机吗？',function(r){
            	    if (r){
            	    	var params = {"ids":ids};
                    	$.post("employee/delete_batch",params, function(data){
                			if(data.status == 200){
                				$.messager.alert('提示','删除司机成功!',undefined,function(){
                					$("#employeeList").datagrid("reload");
                				});
                			}
                		});
