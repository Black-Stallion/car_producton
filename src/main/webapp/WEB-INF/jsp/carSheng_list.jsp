...
<table class="easyui-datagrid" id="carShengList" title="信息列表" 
       data-options="singleSelect:false,collapsible:true,pagination:true,rownumbers:true,url:'carSheng/list',method:'get',pageSize:10,fitColumns:true,toolbar:toolbar_carSheng">
    <thead>
        <tr>
        	<th data-options="field:'ck',checkbox:true"></th>
        	<th data-options="field:'carnumber',width:100,align:'center'">车牌号</th>
        	<th data-options="field:'typename2',align:'center',width:90,formatter:formatCarType2">车型名称</th>
            ...
            <th data-options="field:'formatCarList2',width:130,align:'center',formatter:formatCarList2">查看详细信息</th>
             <th data-options="field:'carShengEdit',align:'center',width:50,formatter:formatCarShengEdit">编辑</th>
        </tr>
    </thead>
</table> 

<div  id="toolbar_carSheng" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
	
	<c:forEach items="${sessionScope.sysPermissionList}" var="per" >
		<c:if test="${per=='carSheng:add' }" >
		    <div style="float: left;">  
		         <a href="#" class="easyui-linkbutton" plain="true" icon="icon-add" onclick="carSheng_add()">新增</a> 
		    </div>  
		</c:if>
		<!-- 
		<c:if test="${per=='carSheng:edit' }" >
		    <div style="float: left;">  
		        <a href="#" class="easyui-linkbutton" plain="true" icon="icon-edit" onclick="carSheng_edit()">编辑</a>  
		    </div>  
		</c:if>
		 -->
		
		<c:if test="${per=='carSheng:delete' }" >
		    <div style="float: left;">  
		        <a href="#" class="easyui-linkbutton" plain="true" icon="icon-cancel" onclick="carSheng_delete()">删除</a>  
		    </div>  
		</c:if>
	</c:forEach>
	...
	<div style="float: left;">  
		<a href="#" class="easyui-linkbutton" plain="true" icon="icon-reload" onclick="carSheng_reload()">刷新</a>  
	</div> 
	<div id="search_carSheng" style="float: right;">
        <input id="search_text_carSheng" class="easyui-searchbox"  
            data-options="searcher:doSearch_carSheng,prompt:'请输入...',menu:'#menu_carSheng'"  
            style="width:250px;vertical-align: middle;">
        </input>
        <div id="menu_carSheng" style="width:120px"> 
			<div data-options="name:'carnumber2'">车牌号</div>
			<div data-options="name:'departmentName2'">所在部门</div> 
			<div data-options="name:'username2'">机动车所有人</div>
		</div>     
    </div>
</div>  

<div id="carShengEditWindow" class="easyui-window" title="编辑车辆" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'carSheng/edit'" style="width:800px;height:590px;padding:10px;">
</div>
<div id="carShengAddWindow" class="easyui-window" title="添加车辆" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'carSheng/add'" style="width:800px;height:590px;padding:10px;">
</div>
<div id="carShengInfo" class="easyui-dialog" title="车型信息" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save'" style="width:770px;height:550px;padding:20px;">
	<form id="carTypeEditForm12" method="post">					
...
<div id="carListWindow2" class="easyui-dialog" title="车辆信息" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save'" style="width:800px;height:590px;padding:10px;">
	<div class="easyui-tabs" style="width:800px;height:560px;">
	<div title="车辆信息" style="width:800px;height:560px;">
	<form id="carSheng1EditForm" method="post">					
	    ...
	    <table  class="border-table" cellpadding="5" algin="center">
	       ...
	       <tr>
	        ...
	        <tr>
	            ...
	            <td>
    				<input class="easyui-textbox" id="departmentzx" name="departmentName" panelHeight="200" size="11px"/>  
    				<input name="departmentId" id="departmentId12" type="hidden"/>
	            </td>
		      </tr>
	        <tr>
	       ...
	       <td  bgcolor="#F0F8FF" align="right">使用性质</td>
	        ...
	        <tr >
	            ...
	            <td colspan="3" >
	            	 <div style="padding-top: 12px"><span id="shengPicSpan"></span></div>
	                 <input type="hidden" class="easyui-linkbutton shengPic" name="image"/>
	            </td>
	        </tr>
	    </table>
	     </center>
	...
	<div title="用油记录" style="width:800px;height:560px;">
			<form id="carYoukaFind1Form21" method="post">
			<center>
	    	<table  class="border-table" cellpadding="5" algin="center">
	        <tr>
		...
		<div title="用车记录" style="width:800px;height:560px;">
		 <table id="carSend2Lists" class="easyui-datagrid">
		</table> 
		</div> 
		<div title="保养记录" style="width:800px;height:560px;">
		 <table id="carKeep2Lists" class="easyui-datagrid">
		</table> 
		</div> 
		<div title="维修记录" style="width:800px;height:560px;">
		 <table id="carKeepApply2Lists" class="easyui-datagrid">
		</table> 
		</div>  
		<div title="事故记录" style="width:800px;height:560px;">
		 <table id="carShigu2Lists" class="easyui-datagrid">
		</table> 
		</div> 
		<div title="违章记录" style="width:800px;height:560px;">
		 <table id="carWeizhang2Lists" class="easyui-datagrid">
		</table> 
		</div>
		<div title="运用记录" style="width:800px;height:560px;">
		 <table id="carUsing2Lists" class="easyui-datagrid">
		</table> 
		</div>
		<div title="月检记录" style="width:800px;height:560px;">
		 <table id="carCheck2Lists" class="easyui-datagrid">
		</table> 
		</div>
		<div title="所换大件记录" style="width:800px;height:560px;">
		 <table id="carBig2Lists" class="easyui-datagrid">
		</table> 
		</div>   
	</div>
</div>
<script>
function doSearch_carSheng(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		$("#carShengList").datagrid({
	        title:'信息列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get', nowrap:true,  
	        toolbar:"toolbar_carSheng", url:'carSheng/list', method:'get', loadMsg:'数据加载中......', fitColumns:true,//允许表格自动缩放,以适应父容器  
	        columns : [ [ 
             	{field : 'ck', checkbox:true }, 
             	{field : 'carnumber', width : 100, title : '车牌号', align:'center'},
             	{field : 'typename2', width : 90, align:'center',title : '车型名称',formatter:formatCarType2},
            	...
            	{field : 'color', width : 80, title : '车辆颜色', align:'center'}, 
             	...
             	{field : 'formatCarList2', width : 130, title : '查看详细信息', align:'center',formatter:formatCarList2},
             	{field : 'carShengEdit', width : 50, align:'center', title : '编辑',formatter:formatCarShengEdit}
        ] ],  
    });
}else{
	$("#carShengList").datagrid({  
        title:'信息列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get', nowrap:true,  
        toolbar:"toolbar_carSheng", url:'carSheng/search_carSheng_by_'+name+'?searchValue='+value, loadMsg:'数据加载中......', fitColumns:true,//允许表格自动缩放,以适应父容器  
        columns : [ [ 
        	{field : 'ck', checkbox:true }, 
         	{field : 'carnumber', width : 100, title : '车牌号', align:'center'},
         	{field : 'typename2', width : 90, align:'center',title : '车型名称',formatter:formatCarType2},
...
//编辑
function  formatCarShengEdit(value, row, index){ 
	return "<a herf='#' style='cursor:pointer' icon='icon-edit' onclick='carSheng_edit()'><span style='color:blue'>编辑</span></a>";
};
  //格式化车型信息
    function  formatCarType2(value, row, index){ 
    	if(value ==null && value == ''){
			return '无';}
    	else{
    		return "<a href=javascript:openCarType2("+index+")>"+row.carType.typename+"</a>";
    	}
	};
	...
	//格式化车辆信息
	 function  formatCarList2(value, row, index){ 
	    	if(value ==null && value == ''){
				return '无';}
	    	else{
	    		return "<a href=javascript:openCarList2("+index+")>"+"车辆详细信息"+"</a>";
	    	}
		};
	 //格式化部门信息
 //   function  formatDepartment(value, row, index){ 
...
//	};
	//根据index拿到该行值
	function onCarShengClickRow(index) {
		var rows = $('#carShengList').datagrid('getRows');
		return rows[index];
	}
	
	//打开车型信息对话框
	function  openCarType2(index){ 
		var row = onCarShengClickRow(index);
		$("#carShengInfo").dialog({
    		onOpen :function(){
    			$.get("carType/get/"+row.carType.typeid,'',function(data){
		    		//回显数据
		    		$("#carTypeEditForm12").form("load",data);
    	    	});
    		},
    	}).dialog("open");
	};
	...
	//打开车辆对话框
	function  openCarList2(index){ 
		var row = onCarShengClickRow(index);
		$("#carListWindow2").dialog({
    		onOpen :function(){
    			$.get("carSheng/get/"+row.carId,'',function(data){
		    		//回显数据
		    		...
		    		$("#carSheng1EditForm").form("load",data);
		    		//加载图片
		    		initShengPic({
		    			"pics" :data.image,
		    		});
    			...
    			$.get("carYouka/get_car/"+row.carId,'',function(data){
        			...
        			$("#carYoukaFind1Form21").form("load", data);
    	    	});
    			////派车单信息
				$("#carSend2Lists").datagrid({
			        ...
			        columns : [ [ 
		             	{field : 'sendnums', width : 80, title : '派车编号', align:'center'},
		...
		$('#carYoukaFind1Form21').form('reset');
	};
	 function initShengPic(data){
	    	$(".shengPic").each(function(i,e){
	    		var _ele = $(e);
	        	...
	        	if(!j){
	    			$("#shengPic").html("<span style='font-size: 12px;font-family: Microsoft YaHei;'>无</span>");
	    		}
	    	});
	    }
		
	function getCarShengSelectionsIds(){
    	var carShengList = $("#carShengList");
    	var sels = carShengList.datagrid("getSelections");
    	var ids = [];
    	for(var i in sels){
	...
	function submitCarTypeEditForm(){
		$.get("carType/edit_judge",'',function(data){
    		if(data.msg != null){
    			...
    			if(!$('#carTypeEditForm12').form('validate')){
    				$.messager.alert('提示','表单还未填写完成!');
    				return ;
    			}
    			$.post("carType/update_all",$("#carTypeEditForm12").serialize(), function(data){
    				if(data.status == 200){
    					$.messager.alert('提示','修改车型成功!','info',function(){
    						$("#carShengInfo").dialog("close");
	...
	function submitCarEditForm(){
		$.get("carSheng/edit_judge",'',function(data){
    		if(data.msg != null){
    			$.messager.alert('提示', data.msg);
    		}else{
    			...
    			$.post("carSheng/update_all",$("#carEditForm").serialize(), function(data){
    				if(data.status == 200){
    					$.messager.alert('提示','修改车辆成功!','info',function(){
    						$("#carListWindow2").dialog("close");
    						$("#carShengList").datagrid("reload");
    					});
    				}else{
    					$.messager.alert('提示',data.msg);
	...
	//更新信息要求
	function updateCarShengNote(){
		$.get("carSheng/edit_judge",'',function(data){
    		if(data.msg != null){
    			$.messager.alert('提示', data.msg);
    		}else{
    			carShengNoteEditor.sync();
    			$.post("carSheng/update_note",$("#carShengNoteForm").serialize(), function(data){
    				if(data.status == 200){
    					$("#carShengNoteDialog").dialog("close");
    					$("#carShengList").datagrid("reload");
    ...
    function carSheng_add(){
    	$.get("carSheng/add_judge",'',function(data){
       		if(data.msg != null){
       			$.messager.alert('提示', data.msg);
       		}else{
       			$("#carShengAddWindow").window("open");
       		}
       	});
    }
    
    function carSheng_edit(){
    	$.get("carSheng/edit_judge",'',function(data){
       		if(data.msg != null){
       			$.messager.alert('提示', data.msg);
       		}else{
       			var ids = getCarShengSelectionsIds();
               	...
               	$("#carShengEditWindow").window({
               		onLoad :function(){
               			//回显数据
               			var data = $("#carShengList").datagrid("getSelections")[0];
               			data.dengjitiem = TAOTAO.formatDateTime(data.dengjitiem);
    		    		data.issuedate = TAOTAO.formatDateTime(data.issuedate);
    		    		data.manufacturedate = TAOTAO.formatDateTime(data.manufacturedate);
    		    		data.departmentId = data.department.departmentId;
            			...
            			//}
    		    		data.typeid = data.carType.typeid; 
               			$("#carShengEditForm").form("load", data);
               			TAOTAO.initShengPicFileUpload({
            				"pics" : data.image,
            			});
               		},
               		onclose :function(){
    ...
    function getCarShengSelectionsNames(){
    	var carShengList = $("#carShengList");
    	var sels = carShengList.datagrid("getSelections");
    	var ids = [];
    	for(var i in sels){
    ...
    function carSheng_delete(){
    	$.get("carSheng/delete_judge",'',function(data){
      		if(data.msg != null){
      			$.messager.alert('提示', data.msg);
      		}else{
      			var names = getCarShengSelectionsNames();
      			var ids = getCarShengSelectionsIds();
              	if(ids.length == 0){
              	...
              	$.messager.confirm('确认','确定删除车牌号为 '+names+' 的信息吗？',function(r){
              	    if (r){
              	    	var params = {"ids":ids};
                      	$.post("carSheng/delete_batch",params, function(data){
                  			if(data.status == 200){
