...
<table class="easyui-datagrid" id="customList" title="司机列表" 
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'customId',width:100,align:'center'">司机编号</th>
            <th data-options="field:'customName',width:100,align:'center'">司机名称</th>
            <th data-options="field:'fullName',width:200,align:'center'">司机全称</th>
            ...
            <th data-options="field:'status',width:60,align:'center',formatter:TAOTAO.formatCustomStatus">司机状态</th>
...
<div  id="toolbar_custom" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
    ...
    <div id="search_custom" style="float: right;">
        ...
        <div id="menu_custom" style="width:120px"> 
			<div data-options="name:'customId'">司机编号</div> 
			<div data-options="name:'customName'">司机名称</div>
		</div>     
    </div>  

</div>

<div id="customEditWindow" class="easyui-window" title="编辑司机" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'custom/edit'" style="width:65%;height:80%;padding:10px;">
</div>

<div id="customAddWindow" class="easyui-window" title="添加司机" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'custom/add'" style="width:65%;height:80%;padding:10px;">
</div>
...
function doSearch_custom(value,name){ //用户输入用户名,点击搜素,触发此函数  
	if(value == null || value == ''){
		$("#customList").datagrid({
	        ...
	        columns : [ [ 
	             	...
	             	{field : 'customId', width : 100, title : '司机编号', align:'center'},
	             	{field : 'customName', width : 100, align : 'center', title : '司机名称'},
	             	{field : 'fullName', width : 200, align : 'center', title : '司机全称'}, 
	            	...
	            	{field : 'email', width : 70, title : '邮箱', align:'center'}, 
	             	...
	             	{field : 'status', width : 60, title : '司机状态', align:'center', formatter:TAOTAO.formatCustomStatus}, 
	...
	function formatCustomNote(value, row, index){ 
		if(value !=null && value != ''){
			return "<a href=javascript:openCustomNote("+index+")>"+"司机介绍"+"</a>";
		}else{
			return "无";
		}
	}
    		...
    		if(data.msg != null){
    			...
    			$.post("custom/update_note",$("#customNoteForm").serialize(), function(data){
    				if(data.status == 200){
    					...
    					$.messager.alert("操作提示", "更新司机介绍成功！");
    				}else{
    					$.messager.alert("操作提示", "更新司机介绍失败！","error");
    				}
    			});
    		}
    	});
