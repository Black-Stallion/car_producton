...
<table class="easyui-datagrid" id="carRegisterList" title="信息列表" 
    ...
    <thead>
        <tr>
            ...
            <th data-options="field:'carPublicEdit',align:'center',width:50,formatter:formatCarPublicEdit">编辑</th>
        </tr>
    </thead>
</table> 
...
<div  id="toolbar_carRegister" style=" height: 22px; padding: 3px 11px; background: #fafafa;">  
	...
	<div style="float: left;">  
	 ...
	 <div id="search_carRegister" style="float: right;">
        <input id="search_text_carPublic" class="easyui-searchbox"  
        ...
        <div id="menu_carRegister" style="width:120px"> 
			...
			<div data-options="name:'departmentName'">所在部门</div> 
			<div data-options="name:'username'">机动车所有人</div>
		</div>     
    </div>  
</div>  

<div id="carRegisterEditWindow" class="easyui-window" title="编辑车辆" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'carPublic/edit'" style="width:800px;height:590px;padding:10px;">
</div>
<div id="carRegisterAddWindow" class="easyui-window" title="添加车辆" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save',href:'carPublic/add'" style="width:800px;height:590px;padding:10px;">
</div>

<div id="carRegisterInfo" class="easyui-dialog" title="车型信息" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save'" style="width:770px;height:550px;padding:20px;">
...
<div id="carListWindow1" class="easyui-dialog" title="车辆信息" data-options="modal:true,closed:true,resizable:true,iconCls:'icon-save'" style="width:800px;height:590px;padding:10px;">
	<div class="easyui-tabs" style="width:800px;height:560px;">
	    ...
	    <table class="border-table" cellpadding="5" algin="center">
	        ...
	        <tr>
	            ...
	            <td><input class="easyui-datetimebox" name="dengjitiem"     
        			data-options="required:true,showSeconds:true" value="date.format('yyyy-MM-dd')" style="width:150px"> </td>
        			<td bgcolor="#F0F8FF" align="right">机动车登记编号</td>
	            <td>
	        ...
	        <tr>
	            ...
	            <td>
    				<input class="easyui-textbox" id="department0059"  name="departmentName" panelHeight="200" size="10px"/>  
	        ...
	        <tr>
	            ...
	            <td colspan="3">
	            	  <div style="padding-top:12px"><span id="publicPicSpan"></span></div>
	                 <input type="hidden" class="easyui-linkbutton publicPic" name="image"/>
	            </td>
	        </tr>
	    </table>
	     </center>
		...
		<div title="用油记录" style="width:800px;height:560px;">
			<form id="carYoukaFindForm21" method="post">
			<center>
	    	<table class="border-table" cellpadding="5" align="center">
	        <tr>
	            <td bgcolor="#F0F8FF" width="120px">车辆性质及车辆</td>
	            <td width="250px">
	            	<input class="easyui-textbox"  name="usenature" editable="false" panelHeight="auto" size="9px" />
	            	<input name="usenature" id="usenature523" type="hidden"/>
    				<input class="easyui-textbox"  name="carnumber" editable="false" panelHeight="auto" size="10px"/>  
		...
		<div title="用车记录" style="width:800px;height:560px;">
		 <table id="carSendLists" class="easyui-datagrid">
		</table> 
		</div> 
		<div title="保养记录" style="width:800px;height:560px;">
		 <table id="carKeepLists" class="easyui-datagrid">
		</table> 
		</div> 
		<div title="维修记录" style="width:800px;height:560px;">
		 <table id="carKeepApplyLists" class="easyui-datagrid">
		</table> 
		</div>  
		<div title="事故记录" style="width:800px;height:560px;">
		 <table id="carShiguLists" class="easyui-datagrid">
		</table> 
		</div> 
		<div title="违章记录" style="width:800px;height:560px;">
		 <table id="carWeizhangLists" class="easyui-datagrid">
		</table> 
		</div>
		<div title="运用记录" style="width:800px;height:560px;">
		 <table id="carUsingLists" class="easyui-datagrid">
		</table> 
		</div>
		<div title="月检记录" style="width:800px;height:560px;">
		 <table id="carCheckLists" class="easyui-datagrid">
		</table> 
		</div>
		<div title="所换大件记录" style="width:800px;height:560px;">
		 <table id="carBigLists" class="easyui-datagrid">
		</table> 
		</div>   
	</div>  
	</div>
	...
	if(value == null || value == ''){
		$("#carRegisterList").datagrid({
	        ...
	        columns : [ [ 
	             	...
	             	{field : 'endDate', width : 100, title : '车牌号', align:'center'},
	            	...
	            	{field : 'color', width : 80, title : '车辆颜色', align:'center'}, 
	             	...
	             	{field : 'formatCarList', width : 130, title : '查看详细信息', align:'center',formatter:formatCarList},
	             	{field : 'carPublicEdit', width : 50, align:'center', title : '编辑',formatter:formatCarPublicEdit}
	        ] ],  
	    });
	}else{
	$("#carRegisterList").datagrid({  
        title:'信息列表', singleSelect:false, collapsible:true, pagination:true, rownumbers:true, method:'get', nowrap:true,  
        toolbar:"toolbar_carRegister", url:'carPublic/search_carPublic_by_'+name+'?searchValue='+value, loadMsg:'数据加载中......', fitColumns:true,//允许表格自动缩放,以适应父容器  
        columns : [ [ 
        	{field : 'ck', checkbox:true }, 
...
//编辑
function  formatCarPublicEdit(value, row, index){ 
	return "<a herf='#' style='cursor:pointer' icon='icon-edit' onclick='carRegister_edit()'><span style='color:blue'>编辑</span></a>";
};
//
...
//	};
	...
	function  openCarList(index){ 
		...
		$("#carListWindow1").dialog({
    		onOpen :function(){
    			...
    			$.get("carYouka/get_car/"+row.carId,'',function(data){
        			...
        			$("#carYoukaFindForm21").form("load", data);
    	    	});
    			//派车单信息
    				$("#carSendLists").datagrid({
		...
		$('#carYoukaFindForm21').form('reset');
	};
	 function initPublicPic(data){
	    	$(".publicPic").each(function(i,e){
	    		var _ele = $(e);
	        	...
	        	if(!j){
	    			$("#publicPic").html("<span style='font-size: 12px;font-family: Microsoft YaHei;'>无</span>");
	    		}
	    	});
	    }
