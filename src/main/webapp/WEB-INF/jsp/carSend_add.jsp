		...
		<center>
			<table class="border-table" cellpadding="5" align="center">
				...
				<tr>
					...
					<td><input class="easyui-combobox" id="departmenCombox069"  name="departmentName" data-options="required:true" panelHeight="200" size="15px" /> 
						<input name="departmentFrist" id="departmentFrist" type="hidden" /> 
						<input class="easyui-combobox" id="xxdepartment069" panelHeight="200" size="15px" />
						<input name="departmentId069" id="departmentId069" type="hidden" />
						<input name="departmentId" id="departmentId0726" type="hidden" />
						<!-- 
						<input class="easyui-combobox" id="departmenCombox069"
						data-options="required:true" panelHeight="auto" size="7px" /> <input
						name="departmentFrist" id="departmentFrist" type="hidden" /> <input
						class="easyui-combobox" id="xxdepartment069" panelHeight="200"  name="departmentName"
						size="12px" /> <input name="departmentId069" id="departmentId069"
						type="hidden" />
						<input
						class="easyui-combobox" id="xxdepartment0726" panelHeight="auto" 
						size="10px" /> <input name="departmentId" id="departmentId0726"
						type="hidden" />
						 -->
						</td>
					...
					<td><input class="easyui-combobox"
						id="xjzcarCombox601" data-options="required:true" name="usenature"
						panelHeight="auto" size="9px" /> <input name="usenature"
						id="usenature601" type="hidden" /> <input class="easyui-combobox" name="carnumber"
						id="xjzcarNumberCombox601" data-options="required:true"
						panelHeight="100" size="10px" /> <input name="carId" id="carId601"
						type="hidden" /></td>
				</tr>
				<tr>
...
<script type="text/javascript">
	...
	$(function() {
		...
		orderAddEditor3 = KindEditor.create("#carSendAddForm [name=road]",{
				resizeType : 1,
	...
	function submitCarSendAddForm() {
		...
		$.post("carSend/insert", $("#carSendAddForm").serialize(), function(
				data) {
			if (data.status == 200) {
	...
	$('#xjzcarCombox601').combobox({
		...
		onSelect : function(data) {
			$('#usenature601').val(data.usenature);
			var fatherdep = $('#usenature601').val();

			$('#xjzcarNumberCombox601').combobox({
				...
				onSelect : function(data1) {
					$('#carId601').val(data1.carId);
				}

			}).combobox('clear');
		}
	});
	$('#departmenCombox069').combobox({
	...
	//	panelHeight : 'auto',
		onSelect : function(data) {
			...
			$('#departmentId069').val(data.departmentId);
			$('#departmentId0726').val(data.departmentId);
			var fatherdep = $('#departmentFrist').val();
			$('#xxdepartment069').combobox({
				url : "department/get_data_second/" + fatherdep,
				editable : false,
				valueField : 'departmentId',
				textField : 'departmentName',
				onSelect : function(data1) {
					$('#departmentId069').val(data1.departmentId);
					$('#departmentId0726').val(data1.departmentId);
			//		var fatherdep = $('#departmentId069').val();
			//		$('#xxdepartment0726').combobox({
			//			url : "department/get_data_third/" + fatherdep,
			//			editable : false,
			//			valueField : 'departmentId',
			//			textField : 'departmentName',
			//			onSelect : function(data2) {
			//				$('#departmentId0726').val(data2.departmentId);
			//			}
			//		}).combobox('clear');
				}
