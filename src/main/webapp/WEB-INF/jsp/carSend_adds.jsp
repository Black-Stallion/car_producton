...
<div style="padding: 10px 10px 10px 10px">
	<form id="carSendAddForms" class="carSendForm" method="post">
		<center>
			<table class="border-table" cellpadding="5" align="center">
				...
				<tr>
					...
					<td width="auto"><input class="easyui-combobox"
						name="sendnums" panelHeight="auto"
				...
				<tr>
					...
					<td><input class="easyui-combobox" id="departmenCombox5831"  name="departmentName" data-options="required:true" panelHeight="200" size="15px" /> 
						<input name="departmentFrist" id="departmentFrist" type="hidden" /> 
						<input class="easyui-combobox" id="xxdepartment5831" panelHeight="200" size="15px" />
						<input name="departmentId5831" id="departmentId5831" type="hidden" />
						<input name="departmentId" id="departmentId4831" type="hidden" />
						<!-- 
						<input class="easyui-combobox" id="departmenCombox5831"
						data-options="required:true" panelHeight="auto" size="7px" /> <input
						name="departmentFrist" id="departmentFrist" type="hidden" /> <input
						class="easyui-combobox" id="xxdepartment5831" panelHeight="200"  name="departmentName"
						size="12px" /> <input name="departmentId5831" id="departmentId5831"
						type="hidden" />
						<input
						class="easyui-combobox" id="xxdepartment4831" panelHeight="auto" 
						size="10px" /> <input name="departmentId" id="departmentId4831"
						type="hidden" />
						 -->
						</td>
					...
					<td><input class="easyui-combobox"
						id="xjzcarCombox6831" data-options="required:true" name="usenature"
						panelHeight="auto" size="9px" /> <input name="usenature"
						id="usenature6831" type="hidden" /> <input class="easyui-combobox" name="carnumber"
						id="xjzcarNumberCombox6831" data-options="required:true"
						panelHeight="100" size="10px" /> <input name="carId" id="carId6831"
						type="hidden" /></td>
				</tr>
				<tr>
					...
					<td><input class="easyui-combobox" name="usercarpeople" value="${username }"
						...
						data-options="valueField:'username',textField:'username',multiple:true,
    						url:'user/find_dataByDep',required:true,width:230" />
					</td>
...
<script type="text/javascript">
	var orderAddEditor23;
	$(function() {
		//加载文件上传插件

		orderAddEditor23 = KindEditor.create("#carSendAddForms [name=road]",{
			resizeType : 1,
	...
	function submitCarSendAddForms() {
		//有效性验证
		if (!$('#carSendAddForms').form('validate')) {
			$.messager.alert('提示', '表单还未填写完成!');
			return;
		}
		orderAddEditor23.sync();

		//ajax的post方式提交表单
		//$("#carSendAddForms").serialize()将表单序列号为key-value形式的字符串
		$.post("carSend/insert", $("#carSendAddForms").serialize(), function(
				data) {
			if (data.status == 200) {
				$.messager.alert('提示', '新增派车单成功'+data.msg);
				clearCarSendADDForms();
				$("#carSendAddWindows").window('close');
	...
	$('#xjzcarCombox6831').combobox({
		...
		onSelect : function(data) {
			$('#usenature6831').val(data.usenature);
			var fatherdep = $('#usenature6831').val();

			$('#xjzcarNumberCombox6831').combobox({
				...
				onSelect : function(data1) {
					$('#carId6831').val(data1.carId);
				}

			}).combobox('clear');
		}
	});
	$('#departmenCombox5831').combobox({
	...
	//	panelHeight : 'auto',
		onSelect : function(data) {
			...
			$('#departmentId5831').val(data.departmentId);
			$('#departmentId4831').val(data.departmentId);
			var fatherdep = $('#departmentFrist').val();
			$('#xxdepartment5831').combobox({
				url : "department/get_data_second/" + fatherdep,
				editable : false,
				valueField : 'departmentId',
				textField : 'departmentName',
				onSelect : function(data1) {
					$('#departmentId5831').val(data1.departmentId);
					$('#departmentId4831').val(data1.departmentId);
			//		var fatherdep = $('#departmentId5831').val();
			//		$('#xxdepartment4831').combobox({
			//			url : "department/get_data_third/" + fatherdep,
			//			editable : false,
			//			valueField : 'departmentId',
			//			textField : 'departmentName',
			//			onSelect : function(data2) {
			//				$('#departmentId4831').val(data2.departmentId);
			//			}
			//		}).combobox('clear');
				}
