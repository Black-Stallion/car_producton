...
public class CarToolController {
	...
	@RequestMapping("/list")
	@ResponseBody
	public EUDataGridResult getList(Integer page, Integer rows, CarTool carTool,HttpSession session) throws Exception{
		String id = (String) session.getAttribute("departmentId");
		EUDataGridResult result = null;
		int id1 =  (int) session.getAttribute("leve");
		if(2 == id1){
			result = carToolService.getList(page, rows, carTool);
		}else{
			result = carToolService.getList1(page, rows, carTool,id);
		}
		return result;
	}
	...
	//搜索
	@RequestMapping("/search_carTool_by_carnumber")
	@ResponseBody
