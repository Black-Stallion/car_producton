...
<div style="padding:10px 10px 10px 10px">
	<form id="carSpecialEditForm" class="carSpecialForm" method="post">
	    ...
	    <table class="border-table" cellpadding="5" algin="center">
	        ...
	        <tr>
	            ...
	            <td>
	            	<input class="easyui-combobox" id="departmenComboxqwe2" name="departmentName" panelHeight="200" />
	            	<input name="departmentFrist" id="departmentFrist" type="hidden"/>
    				<input name="departmentId" id="departmentId96" type="hidden"/>
    				<!-- 
    				<input class="easyui-combobox" id="departmenComboxqwe2" value="${fathername }" panelHeight="auto" size="8px" />
	            	<input name="departmentFrist" id="departmentFrist" type="hidden"/>
    				<input class="easyui-combobox" id="departmentqwe2" name="departmentName"  panelHeight="200" size="11px"/>  
    				<input name="departmentId" id="departmentId96" type="hidden"/>
    				 -->
	            </td>
		      </tr>
	...
	<div style="padding:5px 0 0 650px">
	    <a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitCarSpecialEditForm()">提交</a>
	</div>
</div>
<script type="text/javascript">
	
	function submitCarSpecialEditForm(){
		if(!$('#carSpecialEditForm').form('validate')){
			$.messager.alert('提示','表单还未填写完成!');
			return ;
		}
		
		$.post("carSpecial/update_all",$("#carSpecialEditForm").serialize(), function(data){
			if(data.status == 200){
				$.messager.alert('提示','修改车辆信息成功!','info',function(){
					$("input[name='image']").val(null);
					$("#carSpecialEditWindow").window('close');
	    ...
	    $('#departmenComboxqwe2').combobox({  
	      ...
	      //  panelHeight:'auto',  
	        onSelect : function(data){  
	            ...
	            $('#departmentId96').val(data.departmentId);  
	    //        var fatherdep= $('#departmentFrist').val();
	    //        $('#departmentqwe2').combobox({  
	    //            url:"department/get_data_second/"+fatherdep, 
	    //            editable:false,  
	    //            valueField:'departmentId',  
	    //            textField:'departmentName' ,
	     //           onSelect : function(data1){  
	    //              $('#departmentId96').val(data1.departmentId);  
	    //            }
	     //       }).combobox('clear');  
	        }  
	    });  
