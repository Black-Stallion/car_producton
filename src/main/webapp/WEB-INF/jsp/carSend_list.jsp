...
<table class="easyui-datagrid" id="carSendList" title="信息列表" 
    ...
    <thead>
        <tr>
            ...
            <th data-options="field:'carSendEdit',align:'center',width:50,formatter:formatCarSendEdit">编辑</th>
        </tr>
    </thead>
</table> 
...
function doSearch_carSend(value,name){ //用户输入用户名,点击搜素,触发此函数  
	...
	if(value == null || value == ''){
		$("#carSendList").datagrid({
	        ...
	        columns : [ [ 
	            	...
	            	{field : 'drivername', width : 80, title : '驾驶员', align:'center',formatter:formatDriver}, 
	             	...
	             	{field : 'carSendEdit', width : 50, align:'center', title : '编辑',formatter:formatCarSendEdit}
	        ] ],  
	    });
	}else{
		$("#carSendList").datagrid({
	 ...
	 function  formatChepai(value, row, index){ 
		...
		function  formatCarSendEdit(value, row, index){ 
			return "<a herf='#' style='cursor:pointer' icon='icon-edit' onclick='carSend_edit()'><span style='color:blue'>编辑</span></a>";
		};
